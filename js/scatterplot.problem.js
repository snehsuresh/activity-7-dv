function scatter_plot(data, ax, title = "", xCol = "", yCol = "", rCol = "", legend = [], colorCol = "", margin = 50) {
    ...
    const legendItems = legendContainer.selectAll(".legend-item")
        ...
        .on("click", function (event, country) {
            const isActive = d3.select(this).classed("hidden");
            // Toggle the "hidden" class on the legend item to show/hide it
            d3.select(this).classed("hidden", !isActive);

            // Toggle opacity of the scatter points (the circles) for the selected category
            d3.selectAll(`.${country}`)
                .transition()
                .style("opacity", isActive ? 1 : 0);  // Toggle opacity of circles

            // Update opacity of the corresponding legend items in all plots
            d3.selectAll(`.legend-item`)
                .style("opacity", isActive ? 1 : 0.5); // Update opacity in all legend items
        });
